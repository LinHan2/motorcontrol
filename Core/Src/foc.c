...
void commutate(ControllerStruct *controller, EncoderStruct *encoder)//电流环 控制
{
...
void torque_control(ControllerStruct *controller){
	// don't use filter for RF company
    // controller->t_ff_filt = 0.9f*controller->t_ff_filt + 0.1f*controller->t_ff;
    controller->t_ff_filt = controller->t_ff;
    float torque_des = controller->kp*(controller->p_des - controller->theta_mech) + controller->t_ff_filt + controller->kd*(controller->v_des - controller->dtheta_mech);//p_des等参数是command联合体中的数
